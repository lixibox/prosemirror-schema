...
var baseListCommand__namespace = /*#__PURE__*/_interopNamespaceDefault(baseListCommand);
...
const createInputRule$1 = (match, handler, isBlockNodeRule = false) => defaultInputRuleHandler(new prosemirrorInputrules.InputRule(match, handler), isBlockNodeRule);
...
function mapFragment(content, callback, parent) {
  ...
  return prosemirrorModel.Fragment.fromArray(children);
}
function mapSlice(slice, callback) {
  const fragment = mapFragment(slice.content, callback);
  return new prosemirrorModel.Slice(fragment, slice.openStart, slice.openEnd);
}
...
function createInputRule(regexp, nodeType) {
  return prosemirrorInputrules.wrappingInputRule(regexp, nodeType, {}, (_, node) => node.type === nodeType);
}
...
const isInsideListItem = state => {
  ...
  return prosemirrorUtils.hasParentNodeOfType(list_item)(state.selection) && $from.parent.type === paragraph;
};
...
function outdentList() {
  return function (state, dispatch) {
    ...
    if (isInsideListItem(state)) {
      ...
      baseListCommand__namespace.liftListItem // 1. First lift list item
  ...
  return function (state, dispatch) {
    ...
    if (grandParent.content.content.length <= 1 && $from.parent.content.size === 0 && !(grandParent.content.size === 0)) {
      ...
      if (dispatch) {
        ...
        const tr$1 = state.tr.replace($from.before(keepItem ? undefined : -1), $from.after(-3), new prosemirrorModel.Slice(wrap, keepItem ? 3 : 2, 2));
  ...
  return function (state, dispatch) {
    ...
    if (isInsideListItem(state)) {
      ...
      if (canSink(initialIndentationLevel, state)) {
        baseListCommand__namespace.sinkListItem(list_item)(state, dispatch);
      }
      return true;
    }
    return false;
  ...
  return (state, dispatch) => {
    ...
    if (code) {
      ...
      if (!insideCode && atRightEdge) {
        const tr = state.tr.setSelection(prosemirrorState.Selection.near(state.doc.resolve($cursor.pos - 1)));
        if (dispatch) {
...
const insertBlock = (state, nodeType, nodeName, start, end, attrs) => {
  ...
  if ($pos.depth > 1 && !(nodeType === codeBlock && prosemirrorUtils.hasParentNodeOfType(listItem)(state.selection))) {
    return null;
  }
  ...
  // Add new node.
  tr = tr.setSelection(new prosemirrorState.NodeSelection(tr.doc.resolve(start + 1))).replaceSelectionWith(newNode).setSelection(new prosemirrorState.TextSelection(tr.doc.resolve(start + depth)));
  return tr;
};
...
function canCreateParagraphNear(state) {
  ...
  const isNodeSelection = state.selection instanceof prosemirrorState.NodeSelection;
...
function createParagraphNear(append = true) {
  return function (state, dispatch) {
    ...
    tr.setSelection(prosemirrorState.TextSelection.create(tr.doc, insertPos + 1));
    if (dispatch) {
      dispatch(tr);
    }
    return true;
...
function headingRule(nodeType, maxLevel) {
  return prosemirrorInputrules.textblockTypeInputRule(new RegExp('^(#{1,' + maxLevel + '})\\s$'), nodeType, getHeadingLevel);
}
...
function getCodeBlockRules(schema) {
  const threeTildeRule = createInputRule$1(/((^`{3,})|(\s`{3,}))(\S*)$/, (state, match, start, end) => {
    ...
    return prosemirrorUtils.safeInsert(codeBlock)(tr);
  }, true);
...
function createHorizontalRuleInputRule(type) {
  ...
  (state, match, start, end) => {
    ...
    tr = prosemirrorUtils.safeInsert(type.create(), hrPos)(tr);

    // Insert a paragraph node after the horizontal rule
    tr = prosemirrorUtils.safeInsert(state.schema.nodes.paragraph.create(), tr.mapping.map(hrPos + 1))(tr);
    return tr;
  });
}
...
function baseKeyMaps(schema) {
  let keys = prosemirrorCommands.baseKeymap;
  function bind(key, cmd) {
    keys[key] = cmd;
  }
  bind('Mod-z', prosemirrorCommands.chainCommands(prosemirrorInputrules.undoInputRule, prosemirrorHistory.undo));
  bind('Shift-Mod-z', prosemirrorHistory.redo);
  const backspaceComands = prosemirrorCommands.chainCommands(prosemirrorInputrules.undoInputRule, cleanUpAtTheStartOfDocument, prosemirrorCommands.deleteSelection, prosemirrorCommands.joinBackward, prosemirrorCommands.selectNodeBackward);
  ...
  if (schema.marks.strong) {
    bind('Mod-b', prosemirrorCommands.toggleMark(schema.marks.strong));
    bind('Mod-B', prosemirrorCommands.toggleMark(schema.marks.strong));
  }
  if (schema.marks.em) {
    bind('Mod-i', prosemirrorCommands.toggleMark(schema.marks.em));
    bind('Mod-I', prosemirrorCommands.toggleMark(schema.marks.em));
  }
  if (schema.marks.superscript) {
    bind('Shift-Mod-.', prosemirrorCommands.toggleMark(schema.marks.superscript));
  }
  if (schema.nodes.hard_break) {
    let br = schema.nodes.hard_break,
      cmd = prosemirrorCommands.chainCommands(prosemirrorCommands.exitCode, (state, dispatch) => {
  ...
  const enterCommands = [prosemirrorCommands.newlineInCode, prosemirrorCommands.createParagraphNear, prosemirrorCommands.liftEmptyBlock, prosemirrorCommands.splitBlock];
  ...
  bind('Enter', prosemirrorCommands.chainCommands.apply(null, enterCommands));
  bind(modEnter, prosemirrorCommands.chainCommands.apply(null, enterCommands));
...
const toggleBlockType = (type, attrs) => (state, dispatch) => {
  ...
  const setBlockFunction = prosemirrorCommands.setBlockType(newNodeType, attrs);
...
const wrapListItem = (nodeType, options) => cmdItem(baseListCommand.wrapInList(nodeType, options.attrs), options);
const imageUploadItem = (nodeType, onImageUpload) => new prosemirrorMenu.MenuItem({
  title: "Upload image",
  icon: icons.image,
  enable() {
    return true;
...
const linkItem = markType => new prosemirrorMenu.MenuItem({
...
const buildMenuOptions = (schema, {
  ...
  const availableMenuOptions = {
    ...
    undo: new prosemirrorMenu.MenuItem({
      title: "Undo last change",
      run: prosemirrorHistory.undo,
      enable: state => prosemirrorHistory.undo(state),
      icon: icons.undo
    }),
    redo: new prosemirrorMenu.MenuItem({
      title: "Redo last undone change",
      run: prosemirrorHistory.redo,
      enable: state => prosemirrorHistory.redo(state),
      icon: icons.redo
    }),
...
class MessageMarkdownTransformer {
  constructor(schema, tokenizer = md$1) {
    ...
    this.markdownParser = new prosemirrorMarkdown.MarkdownParser(schema, tokenizer, filterMdToPmSchemaMapping(schema, messageMdToPmMapping));
  }
  encode(_node) {
...
class ArticleMarkdownTransformer {
  constructor(schema, tokenizer = md) {
    ...
    this.markdownParser = new prosemirrorMarkdown.MarkdownParser(schema, tokenizer, filterMdToPmSchemaMapping(schema, articleMdToPmMapping));
  }
  encode(_node) {
    throw new Error('This is not implemented yet');
  }
...
const team_mention = (state, node) => {
  const uri = state.esc(`team_mention://team/${node.attrs.teamId}/${encodeURIComponent(node.attrs.teamName)}`);
...
const ArticleMarkdownSerializer = new prosemirrorMarkdown.MarkdownSerializer({
  blockquote,
  code_block,
  heading,
  horizontal_rule,
...
const MessageMarkdownSerializer = new prosemirrorMarkdown.MarkdownSerializer({
  mention,
  team_mention,
  blockquote,
  code_block,
...
const fullSchema = new prosemirrorModel.Schema({
  nodes: {
    doc: prosemirrorMarkdown.schema.spec.nodes.get('doc'),
    paragraph: prosemirrorMarkdown.schema.spec.nodes.get('paragraph'),
    blockquote: prosemirrorMarkdown.schema.spec.nodes.get('blockquote'),
    horizontal_rule: prosemirrorMarkdown.schema.spec.nodes.get('horizontal_rule'),
    heading: prosemirrorMarkdown.schema.spec.nodes.get('heading'),
    code_block: prosemirrorMarkdown.schema.spec.nodes.get('code_block'),
    text: prosemirrorMarkdown.schema.spec.nodes.get('text'),
    image: prosemirrorMarkdown.schema.spec.nodes.get('image'),
    hard_break: prosemirrorMarkdown.schema.spec.nodes.get('hard_break'),
    ordered_list: Object.assign(baseListCommand.orderedList, {
      content: 'list_item+',
      group: 'block'
    }),
    bullet_list: Object.assign(baseListCommand.bulletList, {
      content: 'list_item+',
      group: 'block'
    }),
    list_item: Object.assign(baseListCommand.listItem, {
      content: 'paragraph block*'
    })
  },
  marks: {
    link: prosemirrorMarkdown.schema.spec.marks.get('link'),
    em: prosemirrorMarkdown.schema.spec.marks.get('em'),
    superscript: {
      parseDOM: [{
        tag: 'sup'
      }],
    ...
    strong: prosemirrorMarkdown.schema.spec.marks.get('strong'),
    code: prosemirrorMarkdown.schema.spec.marks.get('code'),
    strike: {
      parseDOM: [{
        tag: 's'
      }, {
...
const messageSchema = new prosemirrorModel.Schema({
  nodes: {
    doc: prosemirrorMarkdown.schema.spec.nodes.get('doc'),
    paragraph: prosemirrorMarkdown.schema.spec.nodes.get('paragraph'),
    blockquote: prosemirrorMarkdown.schema.spec.nodes.get('blockquote'),
    ...
    hard_break: prosemirrorMarkdown.schema.spec.nodes.get('hard_break'),
    image: {
      ...prosemirrorMarkdown.schema.spec.nodes.get('image'),
      attrs: {
        ...prosemirrorMarkdown.schema.spec.nodes.get('image').attrs,
        height: {
          default: null
        }
      },
...
const buildEditor = ({
  ...
  plugins = [],
  enabledMenuOptions
}) => [...(plugins || []), prosemirrorHistory.history(), baseKeyMaps(schema), blocksInputRule(schema), textFormattingInputRules(schema), linksInputRules(schema), hrInputRules(schema), listInputRules(schema), prosemirrorDropcursor.dropCursor(), prosemirrorGapcursor.gapCursor(), Placeholder(placeholder), prosemirrorMenu.menuBar({
  floating: true,
...
exports.ArticleMarkdownSerializer = ArticleMarkdownSerializer;
exports.ArticleMarkdownTransformer = ArticleMarkdownTransformer;
exports.MessageMarkdownSerializer = MessageMarkdownSerializer;
exports.MessageMarkdownTransformer = MessageMarkdownTransformer;
